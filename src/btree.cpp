...
static btree_t* btree_insert_nonfull(btree_t *btree, type_t key, int val);
...
btree_t* btree_insert(btree_t *btree, type_t key, int val)
{
    ...
    return btree_insert_nonfull(btree, key, val);
}
...
btree_t* btree_split_child(btree_t *parent, int pos, btree_t *child)
{
    ...
    for (i = 0; i < M - 1; ++i)
    {
       ...
       z->posm[i] = child->posm[i + M];//edit by Eru
    }
    ...
    for (i = parent->num - 1; i >= pos; --i)
    {
        ...
        parent->posm[i + 1] = parent->posm[i]; //edit by Eru
    }
    parent->key[pos] = child->key[M - 1];
    parent->posm[pos] = child->key[M - 1];
 ...
 * btree is not full  
 */
btree_t* btree_insert_nonfull(btree_t *btree, type_t key, int val)
{
    int i;
    ...
    if (btree->leaf)
    {
        ...
        while (i >= 0 && key < btree->key[i])
        {
            ...
            btree->posm[i + 1] = btree->posm[i];//edit by Eru
            --i;
        }

        btree->key[i + 1] = key;
        btree->posm[i + 1] = val;//edit by Eru
    ...
    else
    {
        ...
        btree_insert_nonfull(btree->child[i], key, val);
    }
...
btree_t* btree_ini(int OramID, int PoM[], int siz)
{
    ...
    for (i = 0; i < siz; i++)
    {
        btree = btree_insert(btree, i * pow(2, OramID), PoM[i]);
    }
